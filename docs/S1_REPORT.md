...
> **Amended after first writing, 2026-08-19.** A hunt of the first named hiding place
> returned after this report was written and **changed R1's last remaining leg**. The
> amendment is finding 3b below and `audit/PRIOR_ART.md` §7.9. It is added rather than
> folded in, so the sequence stays legible.
...
### 3b. And then Q3 — the last thing R1 owned — turned out to be partially occupied too.

*Added after the report was first written. `audit/PRIOR_ART.md` §7.8.7 named three places a
scoop was still hiding; the first was hunted directly and returned late.*

**Ding, Pinson, Hu & Song, *IEEE Transactions on Sustainable Energy* 7(1):163–172 (2016).**
Scenarios generated by NORTA — the inverse CDF of the day-ahead probability forecast is
applied to each component — so **per-lead-time marginals are identical by construction** for
every value of the autoregressive factor ρ, which is swept from −1 to 1 in steps of 0.05.
Priced through a storage decision with a state of charge, efficiencies, SoC bounds and a
two-price imbalance settlement, in which **holding is free**. **Q1 ∧ Q2 ∧ Q3 ⇒ OCCUPIED by
the letter of the rubric.** Pierre Pinson is a co-author — **the third time.**

**Two qualifiers, both from the paper's own text, and R1 now lives entirely inside them.**
Their movement charge is **round-trip efficiency and capacity bounds — physics, not a price
on changing an incumbent decision**; there is no start-up cost, no ramp cost and no
cycling-degradation cost, and they defer degradation to future work. And their headline is a
**null**: their own strategy's profit "remains steady regardless of ρ", with real degradation
appearing only for a naive filter strategy that "will continuously charge or discharge" into
its bounds.

**What survives is well evidenced, and it is now the whole of R1's claim to Q3.** Across a
wide hunt — roughly sixty logged queries and four citation traversals — **no work holds
marginals fixed, measures a temporal path functional, AND prices it through a start-up,
ramping or cycling cost.** `"forecast" + "trajectories" + "start-up" + "cost" + "scenario"`
→ **0**. `"battery" + "scenarios" + "temporal correlation" + (degradation|cycle|cycling) +
cost` → **0**. `"switching cost" + forecast + ("temporal"|autocorrelation|"dependence")` →
**0**. Both Pinson & Girard and Bessa explicitly defer the decision step, and nobody took it
up with a matched-marginal design.

> **State R1's decision leg as a PRICED movement cost, not merely a decision with a state,
> or it is not distinguished.**

**Three further consequences.** The named candidate that could have occupied R1 outright —
**Rachunok et al., *Applied Energy* 268:114986** — is **Q1 = no** and therefore NARROW; that
risk is closed. **One unresolved risk remains:** Chen, Yang, Li & Liu (IEEE Tencon-Spring
2013, doi 10.1109/TENCONSPRING.2013.6584502), whose title is an exact framing match and for
which **no abstract exists in any index**. And **a 1985 paper scores the mechanism on
Q1–Q4** — Williams, Peters & Raiszadeh rearrange demand sequences to differ *only* in serial
correlation and evaluate lot-sizing rules with a real setup cost. Its body was not obtained
and it has no interval or coverage object, but **no sentence in the paper may imply the
mechanism is recent.**

**A third metadata correction, and it is the point of the rule.** Rachunok's volume was
reported as **274** by one agent and **270** by another. **Crossref gives 268.** Both wrong
figures had already been written into this repository before the record was fetched.
...
2. **Of three named hiding places, one is hunted and two are open.** The first — the
   decision-value follow-up Pinson & Girard commissioned in 2012 — was hunted and is
   reported as finding 3b; its named candidate is closed at Q1 = no, but it produced a new
   partial occupant and one unresolved risk (`docs/OUTSTANDING.md` O25b). Still open: the
   applied conformal layer behind publisher bot walls, starting with "AQA"
   (doi 10.1109/CEEPE69795.2026.11552153); and the **Schaake-shuffle /
   ensemble-copula-coupling branch of hydrology and reservoir operations, which is entirely
   absent from this repository** — `Schaake`, `copula coupling`, `variogram` and `PINAW`
   return zero occurrences across the whole tree. ECC preserves the univariate margins
   *exactly by construction*, so every such comparison is automatically matched on coverage
   and width on a real producer, and reservoir operations charge for changing releases.
   **That is the sharpest remaining way R1 falls.** `audit/PRIOR_ART.md` §7.8.7, §7.9.
