...
| **O8** | **Run the Zaffran discriminator**: is turnover a monotone function of mean interval width across the γ grid? | Cheap and decisive. If it is, C1 reduces to Zaffran's ICML 2022 theorem multiplied by a cost rate, and the paper is in serious trouble. Better to know in week one than at review. |
...
| **O17** | **Decompose Ryan's cost sweep (0/5/10/20/50 bps) per losing device.** Ryan applies it only to the two aggregate headline configurations. | This is precisely the analysis Ryan did not do, on real data, and it is the sharpest available statement of F7's contribution. |
