...
| 4 | **Page limit** | ✅ verified, ✅ **met exactly — CORRECTED, see below** | *"Short papers up to 4 pages, excluding references and optional appendices."* The body is **exactly 4 pages**: the `References` heading is the **first content on page 5, offset 0**. 6 pages total, references on pp.5–6. **This row was WRONG when first written on 2026-08-20 and is corrected the same day.** It claimed 4 pages while recording "offset 6" — and offset 6 meant *six lines of body text were spilling onto page 5*, i.e. the body was 4 pages **plus four lines** and over E-values' ceiling. The wave-4 instruction critic caught it by reading page 5 directly instead of trusting the offset field. The offset is now 0 and the claim is true; the criterion to use is **"References is the first content on the page", never a bare page count and never a non-zero offset.** |
