...
%       [S18 SUB-SESSION B, 2026-08-27: BOTH HALVES OF F2 ARE NOW SUPERSEDED.  The restored
%       caveat did not qualify that verdict, it invalidated it -- a post-hoc betting sign is not
%       a test martingale, so Ville's inequality never applied.  sections/appendix.tex now runs
%       the two-sided mixture (1/2)M(+1) + (1/2)M(-1), which needs no direction in advance, and
%       the verdict is the SIXTH interval, not the fifth.  The quotations above are kept as the
%       accurate record of what the text said at S15; they no longer describe the live paper.]
...
% VENUE-DETECTION PASS (2026-08-26): the workshop's own live CFP page
% (https://e-values-workshop.github.io/, fetched fresh, cache-busted, and re-fetched a second
% time to rule out a caching artefact -- both fetches byte-identical) states its own name as
% "E-Values: From Statistics to ML" in three places that agree with each other: the <title>
% tag, the og:title meta tag, and the page's own <h1>.  That is a capital V in "E-Values";
% every prior session's \workshoptitle used a lowercase v ("E-values").  OpenReview's own
% metadata is NOT consistent with either: the domain's `content.title` field reads "NeurIPS
% 2026 Workshop on E-values: From Statistics To ML" (lowercase v, capital T in "To"), and its
% `content.subtitle` reads "NeurIPS 2026 Workshop E-values" (lowercase v).  Three official
% sources, three different capitalisations, and the machine-readable OpenReview group id
% itself ("NeurIPS.cc/2026/Workshop/E-values") is an identifier, not a display string, so it
% settles nothing either way.  This is a single-letter cosmetic discrepancy with no bearing on
% the venue's identity, blind mode, deadline, or page limit, all of which are unambiguous and
% unchanged.  Set to the workshop's own public self-presentation (the page a reader or
% reviewer actually sees), since that is literally what "the CFP itself states" means; if the
% operator prefers to match OpenReview's internal casing instead, revert this one line.
...
% S21: NAME and EMAIL are blanked to explicit fill-in placeholders for this scrubbed build --
% the operator restores the real values in Overleaf as the final step before submission.
% THE AFFILIATION IS [OPERATOR INPUT]: the visible value below is already a placeholder, and
% the question is docs/OPEN_QUESTIONS.md Q11; left as-is per S21's brief (operator's choice
% whether to mark it further, not decided here).
...
% ---------------------------------------------------------------------------
% S17 SUB-SESSION B (2026-08-26): TITLE + ABSTRACT PRECISION PASS.  Wording only --
% zero claims added, zero removed, zero strengthened.  Abstract stays 12 typeset lines
% and the body still ends p.4 with "References" first on p.5 (offset 0); this was the
% binding constraint on every candidate below, so any future rewording must be
% char-budgeted against line 12, not merely "short enough".
% THE TITLE IS KEPT, AND THE REASONING IS RECORDED SO IT IS NOT RE-LITIGATED.
% "Binds" still does what S4 wave 4's critic bought it for: it LOCATES the claim rather
% than asserting it, which is exactly what the four-boundary result does.  "a Measured
% Boundary" was tested for underselling now that both directions are proved, and KEPT:
% the paper positions its own claim as "smaller and more precise" than a rate forfeit
% (sections/forfeit.tex), and a title reading "Proved" would fight that positioning.
% "a Correction" was tested for overclaiming and KEPT: it names the correction the paper
% DELIVERS to the record, and sentence 3 of the abstract disclaims authorship of both
% halves within 40 words of it.
% FOUR ABSTRACT EDITS, each a wording fix for a defect, not a preference:
%   1. "conceded by name" -> "credited by name".  "Concede" reads as admitting a fault in
%      the derivation; the intent is intro.tex's "We claim neither the placement nor the
%      derivation", i.e. attribution.  Same length, so free.
%   2. "At the null scorecaster THAT CONDITION is tight" -> "At the null scorecaster AND
%      THE MINIMAL ADMISSIBLE SATURATOR THAT RADIUS is tight".  TWO defects in one clause.
%      (a) "that condition" had no antecedent -- the nearest noun phrase two sentences back
%      is "an admissible radius", and sentence 3 sits between them; "that radius" points at
%      it directly.  (b) THE SCOPE WAS WRONG, and wrong in the direction S4 wave 4's
%      adversarial critic (N1) already caught once in the TITLE: tightness holds at the null
%      scorecaster PAIRED WITH the minimal admissible saturator, not at the null scorecaster
%      for every admissible saturator.  The tangent integrator at null q-hat forfeits no
%      coverage at any width Corollary 2 permits (sections/forfeit.tex), so the unscoped
%      reading is false on the paper's own numbers.  Sentence 6 already says the edge belongs
%      to the saturator--scorecaster PAIR; naming the saturator here is what makes that land,
%      and it also introduces "saturator" before sentence 6 leans on it.
%   3. "leaves that set at its radius" -> "exits that set at its radius"; ", and past it" ->
%      "; past it".  "Leaves X at Y" is idiomatically "abandons X, positioned at Y" -- the
%      opposite parse from the one meant.  The semicolon pays back 4 chars toward edit 2.
%   4. "the failing band covers" -> "the failing band covers, as measured".  At q-hat = +b/2
%      the body has only a measurement against the SPECIFIED adversary -- (5) permits no width
%      at all there against the mirror adversary -- so the unqualified verb read as a
%      guarantee over the legal class.  "as measured" is sections/forfeit.tex's own idiom for
%      exactly this ("Coverage is retained at every width, as measured").  The qualifier
%      attaches to the +b/2 clause ONLY: the -b/2 clause is proved as well as measured.
% CONSIDERED AND DECLINED, so a later session does not redo the analysis:
%   - Adding "predictable" to "any bounded scorecaster" (sentence 2).  Predictability is
%     definitional for a scorecaster, which is why intro.tex spends its emphasis on the
%     BOUND; the asymmetry with "predictable perturbations" in the same sentence is correct,
%     since a perturbation is not predictable by definition.
%   - Stating the unrestricted necessity direction in the abstract.  Body-supported, but it
%     is a CLAIM the abstract does not currently make, and this pass adds none.
% ---------------------------------------------------------------------------
\begin{abstract}
A printed claim that the scorecaster breaks online conformal coverage meets a
refutation sharing its first and last authors; this paper joins them and locates the edge of the set that
...
threshold inside an admissible radius. The claim was acted on;
the readout's
placement and the corollary's derivation are credited by name.
At the null scorecaster and the minimal admissible saturator, that radius is \emph{tight}. A legal dead band on
the \emph{completed} threshold exits that set at its radius; past it miscoverage is $1.000000$ against
$\alpha = 0.1$: coverage fails outright, not merely its rate. The edge belongs to the
saturator--scorecaster pair, not the readout; elsewhere that radius is sufficient
only. Under the equally legal $\hat{q} \equiv +b/2$ the failing band covers, as measured; at
...
% S16 SUB-SESSION C (2026-08-26): sections/related.tex IS NOT \input HERE.  It is \input AFTER
% sections/appendix.tex again, so \appendix is in force and "Where this sits" renders as
% Appendix~B.  S15 sub-session D brought it into the body on one enabling premise -- TS-LIMITS'
% "4 to 7 pages plus references" counting appendices, so "this section costs the same wherever
% it sits" -- and that premise is void: E-values' call reads "Short papers up to 4 pages,
% excluding references and optional appendices" (re-verified 2026-08-26, byte-identical fetch,
% research/S16/A-venue-reconfirmation.json).  D's merit argument for the body is not disputed;
% it is outranked by the 4-page BODY ceiling, which is the same trade S10 sub-session D made.
% THE MERGE IS NOT UNDONE.  "The four vocabularies" stays merged into that section's opening
% paragraph, word for word: it was a heading over one paragraph, which is a simplification
% unrelated to the venue, and \label{app:bridge} stays retired.  Only the \input point moves.
% The two body pointers at \ref{sec:related} -- sections/intro.tex's roadmap and
% sections/setup.tex's ten-name sentence -- change the noun "Section" to "Appendix" and nothing
% else; the label is untouched.  Record: research/S16/C-appendix-reversal.json.
...
% S21: the sentence used to end "...and this project's audit trail are at [URL]" -- a soft
% self-referential leak (narrates the paper's own multi-session construction) and also simply
% wrong against what the S21 data-release package ships (simulator, frozen config, results,
% generation scripts -- no audit trail). Reworded to name only what is actually shipped.
% ---------------------------------------------------------------------------
\makeatletter
\if@anonymous\else
\section*{Code and data}
The simulator, its frozen configuration, the \texttt{results/} files behind every figure and
table, and the scripts that generate them are at
...
% AMENDED S16 SUB-SESSION C (2026-08-26): there are TWO appendices again -- A, Supporting
% measurements, now holding Table~\ref{tab:forfeit}, Figure~\ref{fig:boundary},
% Figure~\ref{fig:settings} and Table~\ref{tab:stress}, and B, "Where this sits"
% (sections/related.tex), \input below.  sections/evalue.tex is still deleted and is NOT
% restored; the count above was correct only while related.tex was a body section.
...
% S16 SUB-SESSION C (2026-08-26): "Where this sits" sits here again, as Appendix~B, exactly as
% it did from S10 sub-session D through S14.  It is the same file, moved rather than rewritten
% (the four-vocabularies merge S15 sub-session D made inside it is kept); see the note at the
% body \input block above for why, and research/S16/C-appendix-reversal.json for the record.
\input{sections/related}

% ---------------------------------------------------------------------------
% S18 SUB-SESSION C (2026-08-27): NEURIPS PAPER CHECKLIST WIRED IN.  Sub-session A of this
% session verified, against the official template (checklist.tex's own instruction block,
% which states unconditionally "The papers not including the checklist will be desk
% rejected", with no track-scoping conditional) and the venue's own CFP (silent, not a
% waiver), that a checklist IS REQUIRED for this submission. A checklist file existed at
% paper/checklist.tex since an earlier commit but was never \input anywhere, so main.pdf
% was silently shipping with NO checklist at all -- a desk-reject risk this closes.
%
% PLACEMENT.  The template's own instruction (quoted in checklist.tex before this session
% deleted the instruction block per its own "delete this instruction block" directive)
% reads: "The checklist should follow the references and follow the (optional) supplemental
% material." This document's references render via \bibliography{../audit/REFS_VERIFIED}
% above, and its one supplemental section is sections/related.tex ("Where this sits",
% Appendix~B), \input immediately above this block. So the checklist is placed here: after
% both, immediately before \end{document}. THE CHECKLIST DOES NOT COUNT AGAINST THE 4-PAGE
% BODY CEILING -- this is stated explicitly in the template's own instruction block, so no
% page-budget re-verification of the body was needed, only a confirmation that appending it
% does not disturb the body/references boundary (re-verified: body still ends p.4,
% References still opens p.5, exactly as before).
\input{checklist}
