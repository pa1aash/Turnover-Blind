...
% S16 SUB-SESSION D (2026-08-26): S15-E's FRAMING ADDITIONS ARE REVERSED, ITS HEDGES ARE NOT.
% S15-E's own commit describes what it added here as "an orienting sentence, two paragraph breaks
% and one clause naming what the Placement B concession rests on", and states that "every one of
% its eleven hedges is byte-identical" to the pre-S15-E text.  Exactly those four framing items
% are removed, restoring the wording at 351655d^; every hedge, bold run-in and number in this
% section is byte-identical to what stood before this edit, checked item by item against the
% Step-0 inventory in research/S16/D-content-removal.json.  Nothing conceded here is withdrawn.
...
The forfeited certificate was not a tight one. \textbf{The dead-band widths are absolute, and the
...
\textbf{The harness was rebuilt against numbers already read}, and that carries less weight than a
rerun done without sight of them. \textbf{Placement~B is conceded, not tested.} \textbf{The i.i.d.\
check above used one seed} (seed \texttt{20260820}, $T = 10^6$). \textbf{The primary, adversarial harness uses
