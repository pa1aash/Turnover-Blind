...
\textbf{One construction, one adversary, one saturator}, and $0.63/(1-w)$ is specific to the
saturator's \emph{level}, which condition~(4) bounds only from below. \textbf{The
...
$0.75\,b$; under i.i.d.\ scores it returns $0.249376$ at $T = 10^6$, which is not a retreat but an
adversary-free confirmation of the mechanism --- the threshold pins at $b-\tau = 0.5$ and uniform
scores on $[-b/2,b/2]$ put $P(s > 0.5) = 0.25$ exactly. \textbf{The harness was rebuilt against numbers already read}, which carries less
weight than a rerun done without sight of them. \textbf{Placement~B is conceded, not tested.}
\textbf{One seed, and only the null scorecaster's edge is located}; the other four settings are
bracketed at three widths. \textbf{What would overturn the claim} is a legal $(r_t,\hat{q}_t)$ pair with $\tau \le
