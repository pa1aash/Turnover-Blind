...
\begin{equation}
  ...
  \label{eq:pid}
\end{equation}
Angelopoulos et al.'s iteration~(5). Its $r_t$ saturates: their condition~(4) requires $|r_t(x)| \ge b$ with the sign of
$x$ once $|x| \ge c\,h(t)$, for $h$ nonnegative, nondecreasing, and sublinear. With the accumulator
$E_t = \sum_{i \le t}(\mathrm{err}_i-\alpha)$, Proposition~2 gives $|E_T| \le c\,h(T)+1$, read on realised scores.
...
\paragraph{Two readouts, two placements.} Damping a sequentially updated scalar
...
\citep[Eq.~13]{binny2025whomoved}. A proportional cost gives a no-trade band, which moves only when the
target is more than $\tau$ away and then pulls back exactly $\tau$
...
\emph{travel}, $\mathcal{T}_H = \sum_{t=2}^{H}|q_t-q_{t-1}|$, named after actuator travel in process
...
neutral: condition~(4) is a lower bound met with equality here, and Theorem~1 permits scorecasters $b/2$ either
side of null, predictable but not required to be constant in $t$. The adversary is specified, not
tie-broken. It plays $s_t = \mathrm{clip}(\tilde{q}_t+\varepsilon,\pm b/2)$ with $\varepsilon =
10^{-9}$, against the \emph{deployed} $\tilde{q}_t$: the $\varepsilon\downarrow 0$ reading of
``sets the score at the threshold''. That play fires the strict indicator and is legal on both
sides of Theorem~1's $[-b/2,b/2]$, so $b/2+b/2 = b$ is tight. \emph{That specification names one
...
one adversary above;
Section~\ref{sec:forfeit}'s two boundaries, which differ from each other, are quantified over the
whole class.
