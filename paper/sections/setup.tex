...
Conformal PID \citep{angelopoulos2023pid} manipulates it on the score scale,
\begin{equation}
  q_{t+1} \;=\; \hat{q}_{t+1} \;+\; r_t\Big(\textstyle\sum_{i \le t}(\mathrm{err}_i - \alpha)\Big),
  \label{eq:pid}
\end{equation}
their iteration~(5). Its $r_t$ saturates: condition~(4) requires $|r_t(x)| \ge b$ with the sign of
...
\lambda\tilde{q}_t$ \citep{garleanu2013dynamic}, published as post-processing
...
in Figure~\ref{fig:placements}; \textbf{(A)} is measured here.
\emph{The dead band (L1) is not a stylistic alternative to the partial-adjustment (L2) form}; it
is the family whose failure \S\ref{sec:forfeit} characterises. Neither is safe by construction:
L2 forfeits Proposition~2's
rate while covering, L1 past $\tau^{\star}$ loses coverage, and at the legal $\hat{q}_t \equiv
-b/2$ both lose it. What is tight is the radius, not a form.
...
control. The arithmetic is not new. Over a predictive distribution $\sum|\Delta q|$ is the
...
Table~\ref{tab:bridge}'s caption names ten, so a fresh one is used here.
...
Proposition~2 addresses. Neither choice is neutral: (4) is a lower bound met exactly here, and
Theorem~1 permits scorecasters $b/2$ either side of null. The adversary is specified, not
