...
% ===========================================================================
%
% S8 SUB-SESSION D (2026-08-22): FOUR-SURFACE SENTENCE COMPRESSED TO ONE, FOR LINE
% BUDGET.  The body overran onto page 5 by two lines before the References heading
% (measured from a from-HEAD rebuild by sub-session B, who added citations elsewhere
% without worsening it).  This is one of the two compressions authorised to close that
% gap.  What changed: the three-sentence "Four surfaces were queried on 20 August 2026
% ... None returns both the claim and the corollary" became one sentence.  NOTHING WAS
% CUT: the two identifiers, "scorecaster", all four surfaces (arXiv full text, arXiv
% abstracts, OpenReview, the six citing works), \citet{li2024neuralconformal} (still
% \mbox-wrapped, see the S5 sub-session A note above -- do not remove the \mbox), and
% the negative result ("none returns both the claim and the corollary") are all still
% printed. THE DATE IS GONE, ON PURPOSE, NOT AS AN OVERSIGHT: it was a stale specific
% calendar date used to assert currency, which is a separate problem sub-session C was
% independently asked to fix in this same sentence. Dropping the date rather than
% updating it satisfies that concern too, since there is no longer a date to go stale.
% If sub-session C's edit landed first, this compresses that (already-de-dated) version;
% if it lands after, there is nothing left for it to do here.
%
% SECOND PASS, SAME EDIT LOCATION.  Sub-session C's own concurrent edits added roughly
% two lines back to sections/limitations.tex (the seed/determinism split, out of scope
% for this session to touch), so the first pass above was not enough on its own. This
% sentence was tightened further -- "Four surfaces were queried for ... : ... citing
% MACRO, and none returns" became "Both identifiers and ``scorecaster'' were searched
% across ... : ... citers of MACRO: none returns" -- shaving redundant words ("works",
% "records") without dropping any surface, either identifier, "scorecaster", the six
% count, \citet{li2024neuralconformal} (still \mbox-wrapped), or "the claim and the
% corollary" as the stated negative result. See sections/related.tex's matching S8
% sub-session D note for the second half of this pass.
%
% MEASURED, NOT ASSUMED: THIS CUT DOES NOT REACH THE SPILL PAGE. An isolated A/B
% rebuild (this file's compression applied, sections/related.tex held at its pre-S8
% baseline) produced a page-4/5 boundary byte-for-byte identical to the reverse test
% (this file at baseline, sections/related.tex compressed). Figure~1 lands on page~2
% regardless of how many characters this paragraph sheds, and nothing downstream of it
% reflows either -- the saving here is real (this section is measurably shorter) but it
% is absorbed on page~1 rather than reaching the References boundary two sections later.
% The line this session actually recovered on the spill page came from
% sections/related.tex alone; see that file's S8 sub-session D note for the honest
% accounting of what these two authorised compressions did and did not close.
%
% ===========================================================================
% S15 SUB-SESSION E (2026-08-24): DE-COMPRESSION.  The venue is TS-LIMITS, 4-7 pages for the
% WHOLE document, so the 4-page body ceiling that drove S5's, S8's and S10's cuts here no
% longer binds.  Three reversals, each naming the commit whose compression it undoes:
%   1. THE FOUR-SURFACE SENTENCE IS THREE SENTENCES AGAIN (reverses d90a1ef, S8 sub-session D,
%      whose own note says the compression was taken to close a two-line page-4/5 spill and
%      that it "does not reach the spill page" anyway).  Query / surfaces / result, exactly as
%      before that commit.  THE DATE DOES NOT COME BACK: S8 dropped "on 20 August 2026"
%      deliberately, as a stale calendar date asserting currency, and that reason is unaffected
%      by the page budget.  No surface, identifier or count changed; the \mbox stays.
%   2. PROPOSITION 2's FINITE-HORIZON BOUND IS NAMED AGAIN in the Corollary 2 restatement
%      (reverses af018bb, S5 wave 3 / sub-session G1 cut (a), which reduced "conclude
%      |E_T| <= c h(T)+1 = o(T)" to "|E_T| = o(T)" for line budget).  Restored IN WORDS, not in
%      symbols: c and h are defined in sections/setup.tex, not here, so printing the formula in
%      the Introduction would import two undefined symbols to recover one fact.  The retention
%      wording matches sections/forfeit.tex's own ("retains |E_T| <= c h(T)+1").
%   3. THE SECOND CONTRIBUTION HEAD GAINS THE EXPLANATION ITS ONE-LINER DODGED.  "It is a
%      retention result, silent outside that radius" now says what silence means in both
%      directions, and "whether the radius is sharp" now says what sharpness would require.
%      NO CONFIDENCE IS ADDED: "whose edge is the corollary's radius" -> "whose edge coincides
%      with the corollary's radius", which is what (\ref{eq:closed}) states and no more, and the
%      scoping "there" on the necessity claim is untouched.
%   4. The background paragraph's four telegraphic assertions gain connective tissue only.  All
%      nine citation keys, their order, and "None of it is claimed here" are unchanged, and
%      "that movement" still guards the S5 wave 5 / S6 antecedent fix.
% Record: research/S15/E-decompression.json.
% ===========================================================================
% S16 SUB-SESSION C (2026-08-26): THE ROADMAP'S FIRST POINTER READS "Appendix~\ref{sec:related}"
% AGAIN, AND ONLY THE NOUN CHANGES.  sections/related.tex is \input after sections/appendix.tex
% once more, because E-values' ceiling is "4 pages, excluding references and optional
% appendices" and S15 sub-session D's promotion of that section into the body had the
% whole-document TS-LIMITS ceiling as its only enabling premise.  \label{sec:related} is
% unchanged, so the pointer resolves to the same section under the same label; nothing else in
% this paragraph or this file is touched.  Record: research/S16/C-appendix-reversal.json.
% S15 SUB-SESSION D (2026-08-24): THE ROADMAP'S FIRST POINTER READS "Section~\ref{sec:related}"
% AGAIN.  S10 sub-session D changed it to "Appendix" when it exiled that section to pay for
% E-values' 4-page body ceiling, and recorded the revert as one line.  This is that line.  The
% second pointer still reads "Appendix~\ref{sec:evalue}", which is still correct: that section
% stays an appendix, on reader grounds rather than page-budget ones (see sections/evalue.tex).
% Nothing else in this paragraph, and nothing else in this file, is touched.
...
self-correction. The joining is what this paper adds.

% S16 SUB-SESSION D (2026-08-26): S15-E's four de-compressions in this file are reversed to the
% wording at 351655d^ (d90a1ef's and af018bb's compressed forms, and the pre-S15-E second head),
% because E-values' 4-page BODY ceiling binds again.  TWO S15-E CHOICES ARE KEPT: "prices that
% movement" (the S5 wave 5 / S6 antecedent fix, which is a correctness fix, not a de-compression)
% and "coincides with the corollary's radius" (S15-E installed it as the NARROWER reading of
% "is the corollary's radius"; reverting it would strengthen a claim, which is outside licence).
An online conformal method \citep{angelopoulos2021gentle} moves a threshold each round. Forecast
...
\paragraph{The claim and the refutation in print.} Conformal PID \citep{angelopoulos2023pid}
adds a \emph{scorecaster} $\hat{q}_{t+1}$ to a saturating error integrator, asking only that it
be predictable and lie in $[-b/2,b/2]$: ``any function of the past'' (Theorem~1). Anything placed
in that slot is already quantified over. The claim reads ``[s]ince using a scorecaster (D-part) in
C-PID breaks the theoretical coverage guarantee'' \citep{li2024neuralconformal}. It is stated and
acted on in that paper's baselines paragraph, verbatim on refereed printed page 18443. It is refuted
twice, with proof. \citet[Corollary~2]{li2025o2cp} add to the deployed threshold any predictable
$d_t$ with $|d_t| \le \mu_t(b/2-|\hat{q}_t|)$ and $\mu_t \in [0,1]$ predictable. Since
$\hat{q}_t + d_t$ still lies in $[-b/2,b/2]$ it is itself a legal scorecaster, so the result
``has exactly the CPID error-integration form'' and $|E_T| = o(T)$. The same generic-slot argument
carries AcMCP's long-run coverage \citep{wang2026acmcp}. Both identifiers and ``scorecaster'' were
searched across arXiv full text, arXiv abstracts, OpenReview, and Semantic Scholar's six citers of
...
guidance'' while asserting valid coverage: their complaint is model selection, not validity. \textbf{We claim
neither the placement nor the derivation.}

\paragraph{Where Corollary~2 stops, its radius condition binds.} It is a \emph{retention} result, silent
outside that radius, and it leaves open whether the radius is sharp.
A legal perturbation leaves it: the $L_1$ dead-band family
characterised here on the \emph{completed} threshold, whose edge coincides with the corollary's
radius at the null scorecaster (Section~\ref{sec:forfeit}). Past it coverage fails outright: the published
sufficient condition is necessary there. Appendix~\ref{sec:related} sets both claims beside four
vocabularies for the same movement.
% S15 SUB-SESSION D2 (2026-08-25): THE ROADMAP'S SECOND POINTER IS DROPPED, NOT REWORDED.  It
% read "; Appendix~\ref{sec:evalue} reads $E_t$ as a capital process and builds a calibration
% test on it", and sections/evalue.tex is deleted this sub-session on explicit operator
% authorisation to close the page budget.  A roadmap must not promise a section the document no
% longer has.  The betting reading survives in the body, in Section~\ref{sec:forfeit}'s "The
% boundary read as a bet." paragraph, which is signposted by its own heading and needs no
% roadmap entry.  The first pointer, at Section~\ref{sec:related}, is untouched.
