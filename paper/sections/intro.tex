...
An online conformal method \citep{angelopoulos2021gentle} moves a threshold each round, and that
movement is not an unnamed quantity. Forecast stability names, scores and prices it
\citep{tunc2013nervousness,godahewa2025stability,vanbelle2023deepstability,%
pritularga2024congruence,genov2026switching,vanbelle2026stabilizing}. Holding the level fixed and
comparing the temporal structure came earlier still, in forecast verification, which matches
predictive marginals on real producers
\citep{gneiting2007calibration,pinson2012scenarios,worsnop2018scenarios}. A third strand reports
that the two numbers usually printed of a deployed interval, coverage and mean length,
under-describe it \citep{min2026questioning,vaze2026simultaneous}. None of it is
claimed here.
...
twice, with proof. \citet[Corollary~2]{li2025o2cp} add any predictable $d_t$, $|d_t| \le
\mu_t(b/2-|\hat{q}_t|)$ with $\mu_t \in [0,1]$ predictable, to the deployed threshold; since
$\hat{q}_t + d_t$ still lies in $[-b/2,b/2]$ it is itself a legal scorecaster, so the result
``has exactly the CPID error-integration form'' and Proposition~2's own finite-horizon bound on
$|E_T|$ is retained, whence $|E_T| = o(T)$. The same generic-slot argument
carries AcMCP's long-run coverage \citep{wang2026acmcp}. Four surfaces were searched for both
identifiers and ``scorecaster''. They are arXiv full text, arXiv abstracts, OpenReview, and the
six works Semantic Scholar records citing \mbox{\citet{li2024neuralconformal}}. None returns both
the claim and the corollary.
...
\paragraph{Where Corollary~2 stops, its radius condition is tight.} It is a \emph{retention}
result, silent outside that radius: perturbations inside the radius keep the guarantee, and
perturbations outside it are not addressed either way. A sufficient condition can be conservative,
so the question the corollary leaves open is whether its radius is sharp, that is, whether
something legal just outside it actually fails.
...
radius at the null scorecaster. Past it coverage fails outright, not merely its rate: the published
