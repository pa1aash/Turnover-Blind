...
\begin{figure}[tb]
  ...
  \caption{The dead-band coverage cliff at the null scorecaster, and that edge located.
  \textbf{(a)} Realised miscoverage against dead-band width $\tau$ under
  $\hat{q} \equiv 0$ with a saturator of level $b$. Filled markers are $T = 10^{6}$ and
  open markers $T = 2{\times}10^{5}$. The dashed rule is $\tau^{\star} = \sup_x r_t(x) +
  \sup_t \hat{q}_t - b/2$, which is $1$ at this setting.
  \textbf{(b)} Eleven measured widths at the same setting at $T = 10^{5}$, placed by
  $\tau - \tau^{\star}$ on a symlog axis whose linear zone is $|\tau - \tau^{\star}| \le
  10^{-3}$; ticks carry the $\tau$ they mark. The grey band spans the widest covering
  width to the narrowest failing one. Figure~\ref{fig:settings} gives the other four
  $(r_t,\hat{q})$ settings.}
  \label{fig:boundary}
\end{figure}
...
(Figure~\ref{fig:settings}). Under the unbounded tangent integrator
