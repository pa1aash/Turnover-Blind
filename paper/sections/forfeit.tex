...
\begin{figure}[tb]
  ...
  $\tau^{\star-}$, are in Appendix~\ref{app:support}.}
  \label{fig:boundary}
\end{figure}
...
condition, and Table~\ref{tab:stress} separates the two with legal objects only. The supremum can
...
measured at the null scorecaster on eleven widths: it returns between $0.09992$ and $0.09997$ at
$\tau = 0.5$ through $0.999$, and $0.000000$ with $\max_t|E_t| = 10{,}000 = \alpha T$ at $\tau = 1$
and at every wider band. That is $\tau^{\star-} = 1$, closed on the failing side, exactly where
(\ref{eq:boundaries}) puts it. \textbf{So the failure criterion is $|E_T|$, not the miscoverage
...
regime covers there and the mirror adversary does not; Appendix~\ref{app:support} prints both
