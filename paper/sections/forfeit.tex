...
adjustment of weight $w$ leaves it; Table~\ref{tab:forfeit} prices it. For $w = 0.99$ and
$0.999$ the excursion is \emph{flat in $T$} while the bound grows like $\log T$, so the ratio
falls with the horizon. At $w = 0.999$ it is $61.08$ at $T = 10^4$ and $42.10$ at $T = 10^6$. The
forfeit is a property of the smoothing \emph{strength}, not Placement~A, and it buys something:
travel falls from $28{,}311$ to $202$. Its sign is counter-intuitive. A readout on the completed
...
\begin{figure}[tb]
  ...
  \citeauthor{angelopoulos2023pid}'s tangent integrator (arrow). Three widths run under
  ...
  scorecaster's edge is bracketed from both sides. Appendix~\ref{app:support} gives the
...
integrator's ceiling, its realised supremum $b-\tau$ to the printed digit at every $\tau \ge
...
runs agree with that law (Figure~\ref{fig:boundary}); only the null scorecaster's edge is located,
the other four bracketed. \textbf{Here $\sup r_t = b$ and $\hat{q} \equiv 0$, so
...
costs the theorem nothing; it moves the edge too and shrinks the excursion
(Figure~\ref{fig:boundary}). Under the unbounded tangent integrator
...
2.0$, $2.5$, $3.0$ and $5.0$ fail too at $T = 2{\times}10^5$, so the failing set is
$(\tau^{\star},\infty)$, open at the left. \textbf{The endpoint is the one row where the
adversary's convention decides the answer.} At $\tau = \tau^{\star}$ the deployed supremum equals
the adversary's own cap $b/2$. The primary regime then covers, while the exact-threshold play
returns $1.000000$ or $0.000000$ by the tie rule. Every other row is convention-free.
\textbf{And the failing arms have $\mathrm{Sat.} = 1.0000$.} Condition~(4) delivers its full
...
post-processing. Elsewhere it stays sufficient only: at $\hat{q}_t \equiv +b/2$ Corollary~2
