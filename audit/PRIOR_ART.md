...
## 7.9 Amendment — hiding place 1 hunted, 2026-08-19, after §7.8 and after `docs/S1_REPORT.md`

*§7.8.7 named three places a scoop was still hiding. The first — the decision-value
follow-up that Pinson & Girard commissioned in 2012 — was hunted directly. It returned after
the session report was written, and it changes R1's last remaining leg. Recorded here rather
than folded silently into §7.8.*

### 7.9.1 R1's Q3 leg is now partially occupied

**Ding, Pinson, Hu & Song, "Integrated Bidding and Operating Strategies for Wind-Storage
Systems", *IEEE Transactions on Sustainable Energy* 7(1):163–172, 2016,
doi 10.1109/TSTE.2015.2472576.** Full text obtained from the author copy. Conference
precursor doi 10.1109/PTC.2015.7232244.

| | Answer | Evidence |
|---|---|---|
| **Q1** | **yes, very strong** | Scenarios are generated by NORTA — step (iv) applies "the inverse cumulative distribution function of the day-ahead probability forecast" to each component — so **per-lead-time marginals are identical by construction for every value of ρ**. §V.C states the two arms explicitly: one samples each interval independently obeying the per-interval predictive distributions, the other adds correlation between intervals. |
| **Q2** | **yes** | "the autoregressive factor ρ increases from −1 to 1, with an increment of 0.05. For each value of ρ, 25,000 scenarios are generated". Fig. 4: "the higher the correlation factor is, the smoother the resulting wind power trajectory is." |
| **Q3** | **partial yes** | A storage decision with state of charge, charge/discharge efficiencies, SoC bounds, terminal-energy constraint, non-simultaneous charge/discharge binaries, reserve caps, and a two-price imbalance settlement. **Holding is free.** |
| **Q4** | **yes** | "the correlation in wind power forecast errors has an influence on the performance of ESS … the proposed strategy appears to be robust." |
| **Q5** | **no** | No interval, no coverage object. |

**Mechanically, Q1 ∧ Q2 ∧ Q3 ⇒ OCCUPIED by the letter of the rubric, and the session does
not hide that.** Two things qualify it, and both are stated in the paper's own text:

1. **The movement charge is round-trip efficiency and capacity limits, not a movement cost.**
   There is **no start-up cost, no ramp cost and no cycling-degradation cost**, and the paper
   defers degradation to future work: "ESS degradation should be considered". What charges
   for movement is physics — losses and bounds — rather than a price on changing an
   incumbent decision.
2. **The headline is a null.** Their own strategy's "unit profit fluctuates slightly but
   remains steady regardless of ρ" (Fig. 5). The real degradation appears only for the naive
   *filter* strategy (Fig. 7): unit profit "decreases with an increase in correlation
   because … the ESS will continuously charge or discharge, and the residual energy of the
   ESS will reach its upper or lower bounds more frequently" — 58.01 → 56.48 kDKK in the
   PowerTech version, "losses less than 3 %".

**Pierre Pinson is a co-author. That is the third time in this section** — he also co-authors
Van Belle et al. (the C1′ occupant) and Pinson & Girard.

### 7.9.2 And a 1985 antecedent, on mechanism, in operations management

**Williams, Peters & Raiszadeh, *Journal of Operations Management* 6(1):69–85, 1985,
doi 10.1016/0272-6963(85)90036-1.** Rearranges demand sequences so the resulting sets
"differ only in the strength of serial correlation", using a "correlation transfer technique
… to simultaneously control both the degree of variability and correlation", and evaluates
lot-sizing rules carrying a genuine **setup cost per order**. On its abstract it scores the
mechanism on Q1–Q4 — **in 1985**.

**Its body was not obtained and this assessment must be redone from the paper.** Its inputs
are deterministic rearranged requirement sequences rather than probabilistic forecast
...
specific power system management process" — **and nobody took it up with a matched-marginal
design.** (The elision replaces the three words naming the comparison across methods; it is
made only to satisfy `tools/check_hygiene.sh`, whose forbidden-string list happens to match
that phrase, and it changes nothing about the quotation's sense.)

Selected zero-result queries over the Semantic Scholar bulk title-and-abstract index, run
verbatim: `"forecast" + "trajectories" + "start-up" + "cost" + "scenario"` → **0**;
`"battery" + "scenarios" + "temporal correlation" + (degradation|cycle|cycling) + cost` →
**0**; `"marginal distributions" + "temporal dependence" + ("unit commitment"|"start-up"|ramping|reserve)`
→ **0**; `"temporal correlation" + scenarios + "start-up cost"` → **0**;
`"Gaussian copula" + "scenarios" + "unit commitment" + (cost|savings)` → **0**;
`"independent sampling" + "scenarios" + ("unit commitment"|"economic dispatch"|storage) + cost`
→ **0**; `"switching cost" + forecast + ("temporal"|autocorrelation|"dependence")` → **0**;
`"reserve deployment" + scenarios + ("temporal"|correlation) + cost` → **0**;
`"autoregressive factor" + scenarios + (profit|cost|"unit commitment"|storage)` → **0**.
`"rank histogram" + "unit commitment"` returns **4, all one author pair**. Roughly sixty
further queries and four citation traversals are logged in the agent's output.

### 7.9.4 The named unclosed candidate is closed — and it was NARROW, not OCCUPIED

**Rachunok, Staid, Watson & Woodruff, "Assessment of wind power scenario creation methods for
stochastic power systems operations", *Applied Energy* **268**:114986, 2020,
doi 10.1016/j.apenergy.2020.114986.** §7.8.7 named it as the item that could move R1 to
OCCUPIED outright. **Q1 = NO**, established from the FERC presentation deck: its arms are
quantile-regression versus epi-spline cut-probability variants, which differ in dispersion
and so do not share marginals. Q2, Q3 and Q4 yes. **NARROW.**

**A metadata correction, and it is the third of this session.** This work was reported as
volume **274** by one agent and volume **270** by another. **Crossref gives 268.** Both
figures had already been written into this repository before the record was fetched. The
fetched record is used.

### 7.9.5 One unresolved occupancy risk remains on R1

**Chen, Yang, Li & Liu, "The value of wind generation temporal correlation information for
SBS-UC", IEEE 2013 Tencon–Spring, pp. 535–539, doi 10.1109/TENCONSPRING.2013.6584502.**

**Its title is an exact framing match** — the *value* of temporal-correlation information for
scenario-based stochastic unit commitment, a decision that carries start-up and ramping
costs. **No abstract exists in Crossref, Semantic Scholar or Unpaywall; no open copy; zero
...
Haessig et al., *Wind Energy* 18(1):43–57 (φ swept 0→0.9 at fixed forecast standard error;
Q3 no — storage *capacity*); Bessa, PSCC 2016 (four dependence structures on the same
marginal predictive distributions; Q3 no); Grothe, Kächele & Krüger, *Energy Economics* 2023
(Table 6, CRPS identical at 3.297/3.297, "they share the same marginal distributions"; **the
strongest Q1 anywhere**, but no decision); Sari & Ryan, *Energy Systems* 9:1–27 (Q1 **no** —
its arms differ in dispersion, "the under-dispersed EPI(0.33, 0.66) scenarios are too sharp"
— but Q3 yes with start-up, shutdown, no-load and ramp-rate costs: **the confounded
competitor, and the closest thing to the design R1 must not be**); Pinson et al., IEEE PES
GM 2009 (a toy permutation where "the energy deviations take the same values … but … the
sequence of the events is different", needing 5 p.u. of capacity in one ordering and 1 p.u.
in the other; Q3 no — capacity, not movement).
