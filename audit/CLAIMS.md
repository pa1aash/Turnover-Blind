...
| C1b | **WITHDRAWN 2026-08-19.** The claim as written was: *therefore no coverage-based criterion — marginal, conditional or adaptive — can select the adaptation rate for a decision that pays for turnover.* It is condemned claim (iv) in `docs/FRAMING.md` §4 and is quoted here only so the withdrawal is legible. **The operational replacement:** across arms matched on realised coverage and on mean interval width, realised decision cost varies by a measured number of points of annual net log growth, so that pair is uninformative for selecting the movement penalty | withdrawn | n/a | The paper's thesis. Note it is a *negative* claim about a class of criteria, so a demonstration on one method cannot establish it; it needs either an argument that coverage is invariant to the relevant perturbation, or explicit restriction to the tested class. See §7 on DtACI, which selects the step size online *by a coverage-based criterion* and is therefore the sharpest test case. |
| C1c | **WITHDRAWN 2026-08-19.** The claim was to *formalise the turnover-versus-tracking-error frontier*. The word is condemned claim (v) in `docs/FRAMING.md` §4 and is quoted here only so the withdrawal is legible; that ground is held by Vaze (arXiv:2607.26577) and Srinivas (SODA 2026). **The replacement:** report a measured curve over the smoother grid, with no minimax claim and without the word | withdrawn | n/a | Superseded before any work was done against it. |
| C1d | **WITHDRAWN 2026-08-19** with C1c, on which it depends | withdrawn | n/a | Nothing exists yet. Note the plan's own table does not identify a coverage-*optimal* γ — coverage is flat over γ ≥ 0.005, which means coverage does not have an interior optimum to sit at the wrong end of. The claim needs restating as "coverage does not locate a point on the measured turnover–tracking-error curve at all", which is stronger, is what the table shows, and avoids the word "frontier" (see `audit/PRIOR_ART.md` §6, Risk 2). |
